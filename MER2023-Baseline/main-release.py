...
#emos =  ['neutral', 'angry', 'happy', 'sad', 'loving',  'proud', 'excited', 'glad', 'interested', 'afraid', 'fear', 'disgust', 'sorry']
...
#sent2idx, idx2sent = {}, {}
#for ii, sen in enumerate(sent): sent2idx[sen] = ii
#for ii, sen in enumerate(sent): idx2sent[ii] = sen
...
class MERDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.FloatTensor(self.name2audio[name]),\
               ...
               self.name2labels[name]['sent'],\
               self.name2labels[name]['Task'],\
               name
...
def read_names_labels_cmu( label_path, data_type, debug=False):
        names, labels = [], []
        if data_type == 'train': corpus = np.load(label_path, allow_pickle=True)['train_corpus'].tolist()
        ...
        return names, labels
# read (names, labels)
def read_names_labels( label_path, data_type, debug=False):
        names, labels = [], []
        task = 'whole'
        assert task in  ['emo', 'aro', 'val','sent','Task', 'whole']
        assert data_type in ['train', 'test1', 'test2', 'test3']
        ...
        for ii, label in enumerate(labels):
            ...
            val = label['val']
            sent = label['sent']
            labels[ii] = {'emo': emo, 'val': val, 'sent': sent}
        # for debug
        if debug: 
            names = names[:100]
            labels = labels[:100]
...
def get_loaders(args, config):
    #train_dataset = MERDataset(label_path = '/home/slasher/MER2023-Baseline/dataset-process/label-6way.npz',# config.PATH_TO_LABEL[args.train_dataset]
                            #   audio_root = os.path.join( '/home/slasher/MER2023-Baseline/dataset-process/features', args.audio_feature) ,#os.path.join(config.PATH_TO_FEATURES[args.train_dataset], args.audio_feature),
                            #   text_root  = os.path.join( '/home/slasher/MER2023-Baseline/dataset-process/features', args.text_feature) ,#os.path.join(config.PATH_TO_FEATURES[args.train_dataset], args.text_feature),
                             #  video_root = os.path.join( '/home/slasher/MER2023-Baseline/dataset-process/features', args.video_feature),#os.path.join(config.PATH_TO_FEATURES[args.train_dataset], args.video_feature),
                              # data_type  = 'train',
                               #debug      = args.debug)
    data_type = 'train'
    names, labels = read_names_labels(label_path='/home/slasher/MER2023-Baseline/dataset-process/label-6way.npz', data_type= 'train', debug=False)
    ...
    for test_set in args.test_sets:
        #test_dataset = MERDataset(label_path = '/home/slasher/MER2023-Baseline/dataset-process/label-6way.npz',
                                #  audio_root = os.path.join( '/home/slasher/MER2023-Baseline/dataset-process/features', args.audio_feature),
                                 ...
                                 # data_type  = test_set,
                                 # debug      = args.debug)
        names, labels = read_names_labels(label_path='/home/slasher/MER2023-Baseline/dataset-process/label-6way.npz', data_type=test_set, debug=args.debug)
...
def train_or_eval_model(args, model, reg_loss, cls_loss, dataloader, optimizer=None, train=False):
    ...
    for data in dataloader:
        ...
        multi_feat = torch.cat([audio_feat, audio_feat, audio_feat], dim=1)
        ...
        if train:
            ...
            loss3 = cls_loss(sents_out, sents)
...
def overall_metric(emo_fscore, val_mse):
    final_score = val_mse # emo_fscore - val_mse * 0.25
    return final_score
...
def average_folder_results(folder_save, testname):
    ...
    for ii in range(num_folder):
        ...
        for jj in range(len(names)):
            ...
            name2preds[name].append({'emo': emoprob, 'val': valpred, 'sent':sentprob})
    ...
    for name in name2preds:
        ...
        name2avgpreds[name] = {'emo': avg_emopred, 'val': avg_valpred, 'emoprob': avg_emoprob, 'sent': avg_sentpred, 'sentprob': avg_sentpred}
...
if __name__ == '__main__':
    ...
    for setname in args.test_sets:
        ...
        if os.path.exists(label_path):
            if setname in ['test1', 'test2']: emo_fscore, val_mse, final_metric, sent_fscore = report_results_on_test1_test2(label_path, pred_path)
