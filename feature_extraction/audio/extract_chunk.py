...
def extract(model_name, audio_files, save_dir, feature_level, layer_ids=None, gpu=None):
    ...
    # load model
    model_file = os.path.join('/home/slasher/MER2023-Baseline/tools/', f'transformers/{model_name}') #config.PATH_TO_PRETRAINED_MODELS
    ...
    for idx, audio_file in enumerate(audio_files, 1):
        ...
        # Accumulate features for the entire audio file
        accumulated_feature = []
        ...
        for i in range(0, len(samples), chunk_size_samples):
            ...
            input_values = feature_extractor(chunk, sampling_rate=sr, return_tensors="pt").input_values
            ...
            # Accumulate the feature representation of this chunk
            accumulated_feature.append(feature)

        # Combine the features of all chunks
        accumulated_feature = np.concatenate(accumulated_feature, axis=0)

        # Save or process the features for the entire audio file as needed
        csv_file = os.path.join(save_dir, f'{vid}.npy')
        if feature_level == 'UTTERANCE':
            if len(accumulated_feature.shape) != 1: # change [T, C] => [C,]
                accumulated_feature = np.mean(accumulated_feature, axis=0)
            np.save(csv_file, accumulated_feature)
        else:
            np.save(csv_file, accumulated_feature)
...
if __name__ == '__main__':
    ...
    # analyze input
    layer_ids = [-1]
    audio_dir = '/home/slasher/MER2023-Baseline/dataset-process/audio/'#config.PATH_TO_RAW_AUDIO[args.dataset]
    save_dir = "/home/slasher/MER2023-Baseline/dataset-process/features/"#config.PATH_TO_FEATURES[args.dataset]
